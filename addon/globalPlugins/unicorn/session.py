...
from . import callbackCommandsDatabase, local_machine, nvda_patcher, transport, unicorn
...
class RemoteSession(object):
    ...
    def __init__(
        self, local_machine: local_machine.LocalMachine, transport: transport.Transport
    ):
        self.local_machine = local_machine
        self.patcher = None
        self.transport = transport
        self.transport.callback_manager.register_callback(
            "msg_passthrough_gesture", self.local_machine.passthrough_gesture
        )

    def passthroughGesture(self, gestureIdentifier: str):
        self.transport.send(
            type="passthrough_gesture", gestureIdentifier=gestureIdentifier
        )
...
class SlaveSession(RemoteSession):
    ...
    def __init__(self, *args, is_secondary: bool = False, **kwargs):
        ...
        self.transport.callback_manager.register_callback(
            "msg_client_joined", self.handle_client_connected
        )
        self.transport.callback_manager.register_callback(
            "msg_client_left", self.handle_client_disconnected
        )
        self.masters = defaultdict(dict)
        self.master_display_sizes = []
        self.transport.callback_manager.register_callback(
            "transport_disconnected", self.handle_disconnected
        )
        self.transport.callback_manager.register_callback(
            "transport_closing", self.handle_transport_closing
        )
        self.patcher = nvda_patcher.NVDASlavePatcher(is_secondary=is_secondary)
        self.patch_callbacks_added = False
        self.transport.callback_manager.register_callback(
            "msg_channel_joined", self.handle_channel_joined
        )
        self.transport.callback_manager.register_callback(
            "msg_set_braille_info", self.handle_braille_info
        )
        self.transport.callback_manager.register_callback(
            "msg_set_display_size", self.set_display_size
        )
        if versionInfo.version_year >= 2023:
            braille.filter_displaySize.register(
                self.local_machine.handle_filter_displaySize
            )
        ...
        self.transport.callback_manager.register_callback(
            "msg_callbackCommandBounce", self.callbackCommandBounce
        )
    ...
    def handle_channel_joined(
        self, channel=None, clients=None, origin=None, **kwargs
    ) -> None:
        if clients is None:
            clients = []
    ...
    def handle_braille_info(
        self, origin: int, name: str = "", numCells: int = 0, **kwargs
    ) -> None:
    ...
    def _filterUnsupportedSpeechCommands(
        self, speechSequence: speech.SpeechSequence
    ...
    def speak(
        self, speechSequence: speech.SpeechSequence, priority: speech.priorities.Spri
    ) -> None:
        self.transport.send(
            type="speak",
...
class MasterSession(RemoteSession):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.patch_callbacks_added = False
        self.transport.callback_manager.register_callback(
            "msg_speak", self.local_machine.speak
        )
        self.transport.callback_manager.register_callback(
            "msg_cancel", self.local_machine.cancel_speech
        )
        self.transport.callback_manager.register_callback(
            "msg_tone", self.local_machine.beep
        )
        self.transport.callback_manager.register_callback(
            "msg_wave", self.local_machine.play_wave
        )
        self.transport.callback_manager.register_callback(
            "msg_display", self.local_machine.display
        )
        self.transport.callback_manager.register_callback(
            "msg_client_joined", self.handle_client_connected
        )
        ...
        self.transport.callback_manager.register_callback(
            "msg_send_braille_info", self.send_braille_info
        )
