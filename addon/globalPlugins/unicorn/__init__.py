...
from .transport import DVCTransport, RelayTransport
...
class GlobalPlugin(GlobalPlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        winAPI.secureDesktop.post_secureDesktopStateChange.register(
            self.onSecureDesktopChange
        )
    ...
    def create_menu(self) -> None:
        self.menu = wx.Menu()
        self.connect_master_item = self.menu.Append(wx.ID_ANY, _("Connect to remote"))
        self.connect_master_item.Enable(unicorn.unicorn_client())
        gui.mainFrame.sysTrayIcon.Bind(
            wx.EVT_MENU, skipEventAndCall(self.connect_master), self.connect_master_item
        )

        self.disconnect_master_item = self.menu.Append(
            wx.ID_ANY, _("Disconnect from remote")
        )
        self.disconnect_master_item.Enable(False)
        gui.mainFrame.sysTrayIcon.Bind(
            wx.EVT_MENU,
            skipEventAndCall(self.disconnect_master),
            self.disconnect_master_item,
        )

        self.connect_slave_item = self.menu.Append(wx.ID_ANY, _("Connect to local"))
        gui.mainFrame.sysTrayIcon.Bind(
            wx.EVT_MENU, skipEventAndCall(self.connect_slave), self.connect_slave_item
        )

        self.disconnect_slave_item = self.menu.Append(
            wx.ID_ANY, _("Disconnect from local")
        )
        self.disconnect_slave_item.Enable(False)
        gui.mainFrame.sysTrayIcon.Bind(
            wx.EVT_MENU,
            skipEventAndCall(self.disconnect_slave),
            self.disconnect_slave_item,
        )
        ...
        self.submenu_item = gui.mainFrame.sysTrayIcon.menu.Insert(
            2, wx.ID_ANY, _("UnicornDVC"), self.menu
        )

    def terminate(self) -> None:
        winAPI.secureDesktop.post_secureDesktopStateChange.unregister(
            self.onSecureDesktopChange
        )
        if versionInfo.version_year >= 2023:
        ...
        if self.submenu_item is not None:
            try:
                ...
                AttributeError
            ):  # We can get this somehow from wx python when NVDA is shuttingdown, just ignore
                pass
        ...
        if (
            dialogs.UnicornPanel
            in gui.settingsDialogs.NVDASettingsDialog.categoryClasses
        ):
    ...
    def connect_master(self) -> None:
        ...
        try:
            maxBytes = (
                conf["unicorn"]["maxBytes"]
                if conf["unicorn"]["limitMessageSize"]
                else 10**9
            )
            transport = DVCTransport(
                serializer=serializer.JSONSerializer(),
                connection_type=unicorn.CTYPE.CLIENT,
                maxBytes=maxBytes,
            )
        ...
        transport.callback_manager.register_callback(
            "transport_connection_in_trial_mode", self.on_connected_in_trial_mode
        )
        transport.callback_manager.register_callback(
            "transport_trial_expired", self.on_trial_expired
        )
        transport.callback_manager.register_callback(
        ...
        transport.callback_manager.register_callback(
            "msg_client_joined", lambda **kwargs: self.evaluate_remote_shell()
        )
        transport.callback_manager.register_callback(
            "update_plugin_dialog", self.on_connection_status_plugin_changed
        )
        transport.callback_manager.register_callback(
            "update_applib_dialog", self.on_connection_status_appllib_changed
        )
        transport.callback_manager.register_callback(
            "update_nvda_dialog", self.on_connection_status_nvda_changed
        )

        self.master_session = MasterSession(
            transport=transport, local_machine=self.local_machine
        )
    ...
    def connect_slave(self) -> None:
        ...
        try:
            ...
            transport = DVCTransport(
                serializer=serializer.JSONSerializer(),
                connection_type=unicorn.CTYPE.SERVER,
                maxBytes=maxBytes,
            )
        except RuntimeError as e:
            self.on_initialize_failed(e)
            ...
            self.on_initialize_failed(e)
            return
        self.slave_session = SlaveSession(
            transport=transport,
            local_machine=self.local_machine,
            is_secondary=bool(self.master_transport),
        )
        ...
        self.slave_transport.callback_manager.register_callback(
            "msg_set_braille_info", self.send_braille_info_to_master
        )
        self.slave_transport.callback_manager.register_callback(
            "update_applib_dialog", self.on_connection_status_appllib_changed
        )
        ...
        self.slave_transport.callback_manager.register_callback(
            "transport_disconnected", self.on_disconnected_as_slave
        )
        ...
        transport.callback_manager.register_callback(
            "transport_connection_failed", self.on_connected_as_slave_failed
        )
    ...
    def disconnect_master(self) -> None:
        self.callback_manager.call_callbacks(
            "transport_disconnected", connection_type=unicorn.CTYPE.CLIENT
        )
    ...
    def disconnect_slave(self) -> None:
        self.callback_manager.call_callbacks(
            "transport_disconnected", connection_type=unicorn.CTYPE.SERVER
        )
    ...
    def on_connected_as_master(self) -> None:
        ...
        self.callback_manager.call_callbacks(
            "transport_connected",
            connection_type=unicorn.CTYPE.CLIENT,
            transport=self.master_transport,
        )
        self.evaluate_remote_shell()
        ui.message(
            _("Unicorn connection established!"),
    ...
    def on_disconnected_as_master(self) -> None:
        ...
        ui.message(
            _("Unicorn connection interrupted"),
            speechPriority=speech.priorities.Spri.NOW,
        )
    ...
    def on_connected_as_slave(self) -> None:
        log.info("Connected DVC in remote mode")
        self.callback_manager.call_callbacks(
            "transport_connected",
            connection_type=unicorn.CTYPE.SERVER,
            transport=self.slave_transport,
        )
        ui.message(
            _("Unicorn connection established!"),
            speechPriority=speech.priorities.Spri.NOW,
    ...
    def isRemoteShell(self, fg, focus) -> bool:
        ...
        if focus.windowClassName in REMOTE_SHELL_CLASSES:
            return True
        if focus.appModule.appName == "vmware-view" and (
            focus.windowClassName.startswith("ATL")
            or focus.windowClassName.startswith(
                "VMware.Horizon.Client.Sdk:RemoteWindow"
            )
        ):
            return True
    ...
    def on_initialize_failed(self, e: Exception) -> None:
        ...
        wx.CallAfter(
            ...
            message=_(
                "Can't initialize UnicornDVC to create a virtual channel. Consult the log or contact support@babbage.com. \nInternal error: {error}"
            ).format(error=e),
    ...
    def on_connected_as_master_failed(self) -> None:
        ...
        if self.master_transport.successful_connects == 0:
            ...
            gui.messageBox(
                ...
                # Translators: Message shown when cannot connect to the remote computer.
                message=_(
                    "Unable to connect to the virtual channel. Please make sure that your client is set up correctly"
                ),
                style=wx.OK | wx.ICON_WARNING,
            )
    ...
    def on_connected_in_trial_mode(self) -> None:
        ...
        gui.messageBox(
            ...
            message=_(
                "UnicornDVC is running in trial mode. If this message is unexpected, please check whether you have a valid license."
            ),
            style=wx.OK | wx.ICON_WARNING,
        )
    ...
    def on_connected_as_slave_failed(self) -> None:
        if self.slave_transport.successful_connects == 0:
            ...
            gui.messageBox(
                ...
                # Translators: Message shown when cannot connect to the remote computer.
                message=_(
                    "Unable to connect to the virtual channel. Please make sure that you are in a remote session and that your client is set up correctly"
                ),
                style=wx.OK | wx.ICON_WARNING,
            )
    ...
    def set_receiving_braille(self, state: bool) -> None:
        if (
            state
            and self.master_session
            and self.master_session.patch_callbacks_added
            and braille.handler.enabled
        ):
    ...
    def event_gainFocus(
        self, obj: NVDAObjects.NVDAObject, nextHandler: Callable
    ) -> None:
        self.evaluate_remote_shell()
        nextHandler()
    ...
    def enter_secure_desktop(self) -> None:
        ...
        self.sd_relay.callback_manager.register_callback(
            "msg_client_joined", self.sd_on_master_display_change
        )
        self.slave_transport.callback_manager.register_callback(
            "msg_set_braille_info", self.sd_on_master_display_change
        )
    ...
    def sd_on_master_display_change(self, **kwargs) -> None:
        self.sd_relay.send(
            type="set_display_size", sizes=self.slave_session.master_display_sizes
        )
    ...
    def mapPassthroughGestures(self):
        ...
        if self.master_transport:
            unicornMapping = os.path.join(
                os.path.join(NVDAState.WritePaths.addonsDir, "unicornNVDA"),
                "extraMappingMaster.ini",
            )
        if self.slave_transport:
            unicornMapping = os.path.join(
                os.path.join(NVDAState.WritePaths.addonsDir, "unicornNVDA"),
                "extraMappingSlave.ini",
            )
        inputCore.manager.userGestureMap.load(unicornMapping)

    @scriptHandler.script(description="passThrough")
