...
from . import callback_manager, serializer, unicorn
...
class RelayTransport(TCPTransport):
    ...
    def __init__(
        ...
        self.callback_manager.register_callback(
            "transport_connected", self.on_connected
        )
    ...
    def on_connected(self):
        ...
        if self.channel is not None:
            self.send(
                "join", channel=self.channel, connection_type=self.connection_type
            )
        else:
...
class DVCTransport(Transport, unicorn.UnicornCallbackHandler):
    ...
    def run(self) -> None:
        ...
        if res >= 1 << 31:
            ...
            raise ctypes.WinError(res)
        if (
            self.connection_type == unicorn.CTYPE.CLIENT
            and not unicorn.unicorn_client()
        ):  # Master
            self.callback_manager.call_callbacks("transport_connection_failed")
            raise ctypes.WinError(res)
        self.opened = True
    ...
    def parse(self, line):
        ...
        if "type" not in obj:
            return
        if "sequence" in obj:
            obj["sequence"] = self.replaceCallbacksPlaceholdersWithActualCallbacks(
                obj["sequence"]
            )
        callback = "msg_" + obj["type"]
        del obj["type"]
    ...
    def replaceCallbacksPlaceholdersWithActualCallbacks(self, sequence: List) -> List:
        ...
        for item in sequence:
            if isinstance(item, serializer.callBackCommandBounce):
                newSequence.append(
                    self.makeCallBackCommandWrapper(item.compName, item.index)
                )
            else:
    ...
    def send_queue(self) -> None:
        while True:
            ...
            if ctypes.sizeof(strbuf) > self.maxBytes:
                log.error(
                    f"data packet is to big for unicorn to handle! package: \n {item}"
                )
                continue
            res = self.lib.Write(
                ctypes.sizeof(strbuf),
                ctypes.cast(strbuf, ctypes.POINTER(ctypes.wintypes.BYTE)),
            )
            if res:
    ...
    def handle_p2p(self, version: int, **kwargs) -> None:
        if version == PROTOCOL_VERSION:
            self.send(
                type="client_joined",
                client=dict(id=-1, connection_type=self.connection_type),
            )
        else:
    ...
    def _OnDataReceived(
        self, cbSize: int, pBuffer: ctypes.POINTER(ctypes.wintypes.BYTE)
    ) -> int:
        pBuffer = ctypes.cast(
            pBuffer,
            ctypes.POINTER(ctypes.c_wchar * (cbSize // ctypes.sizeof(ctypes.c_wchar))),
        )
...
class ConnectorThread(threading.Thread):
    ...
    def __init__(
        ...
        connector: Transport,
        connect_delay: int = 5,
        run_except: type(OSError) = socket.error,
    ):
        super().__init__()
